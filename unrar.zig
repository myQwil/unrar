...
pub const Header = extern struct {
	...
	pub fn read(self: *Header, data: *Archive) Error!bool {
	...
	extern fn RARReadHeader(*Archive, *Header) Result;
};
...
pub const HeaderEx = extern struct {
	...
	pub fn read(self: *HeaderEx, data: *Archive) Error!bool {
	...
	extern fn RARReadHeaderEx(*Archive, *HeaderEx) Result;
};
...
pub const Archive = opaque {
	...
	extern fn RARCloseArchive(*Archive) Result;
	...
	extern fn RARProcessFile(*Archive, Operation, ?[*:0]u8, ?[*:0]u8) c_int;
	...
	extern fn RARProcessFileW(*Archive, Operation, ?[*:0]WChar, ?[*:0]WChar) c_int;

	pub fn next(self: *Archive) Error!?Header {
		var head: Header = undefined;
		return if (try head.read(self)) head else null;
	}

	pub fn nextEx(self: *Archive) Error!?HeaderEx {
