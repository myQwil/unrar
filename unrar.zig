...
fn toError(i: c_uint) Error {
	return switch(@as(ErrorCode, @enumFromInt(i))) {
		.no_memory => Error.OutOfMemory,
		.bad_data => Error.BadData,
		.bad_archive => Error.BadArchive,
		.unknown_format => Error.UnknownFormat,
		.eopen => Error.Open,
		.ecreate => Error.Create,
		.eclose => Error.Close,
		.eread => Error.Read,
		.ewrite => Error.Write,
		.small_buf => Error.SmallBuffer,
		.unknown => Error.Unknown,
		.missing_password => Error.MissingPassword,
		.ereference => Error.Reference,
		.bad_password => Error.BadPassword,
		.large_dict => Error.LargeDictionary,
	};
}
